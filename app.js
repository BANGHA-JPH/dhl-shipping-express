...
const COORDINATES = {
  ...
  LEJ: { lat: 51.4239, lng: 12.2364, label: "Leipzig Global Hub (LEJ)" },
  CVG: { lat: 39.0488, lng: -84.6678, label: "Cincinnati Americas Hub (CVG)" },
  FRA: { lat: 50.0379, lng: 8.5622, label: "Frankfurt Gateway (FRA)" }
};
...
function initDatabase() {
  if (!localStorage.getItem("dhl_shipments") && !localStorage.getItem("ups_shipments")) {
    localStorage.setItem("dhl_shipments", JSON.stringify(DEFAULT_SHIPMENTS));
  }
  if (!localStorage.getItem("dhl_customers") && !localStorage.getItem("ups_customers")) {
    localStorage.setItem("dhl_customers", JSON.stringify(DEFAULT_CUSTOMERS));
  }
...
class AppState {
  static getShipments() {
    return JSON.parse(localStorage.getItem("dhl_shipments") || localStorage.getItem("ups_shipments") || "[]");
  }
  
  static saveShipments(shipments) {
    localStorage.setItem("dhl_shipments", JSON.stringify(shipments));
  }

  static getCustomers() {
    return JSON.parse(localStorage.getItem("dhl_customers") || localStorage.getItem("ups_customers") || "[]");
  }

  static saveCustomers(customers) {
    localStorage.setItem("dhl_customers", JSON.stringify(customers));
  }
  ...
  static getActiveUserEmail() {
    ...
    if (role === "admin") return "admin@dhl.com";
    if (role === "customer") return "customer@dhl.com";
    return "";
  }
...
class Router {
  ...
  initAdminAppointment() {
    ...
    if (trackingInput && !trackingInput.value) {
      trackingInput.value = "DHL-" + Math.floor(10000000 + Math.random() * 90000000);
    }
...
class SimulationController {
  constructor() {
    this.activeShipmentId = localStorage.getItem("sim_current_id") || "DHL-78361092";
    this.timerId = null;
  ...
  registerShippingAppointment() {
    ...
    document.getElementById("appt-tracking").value = "DHL-" + Math.floor(10000000 + Math.random() * 90000000);
