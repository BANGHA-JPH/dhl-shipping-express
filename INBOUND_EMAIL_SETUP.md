...
This guide explains how customer email replies to `support@dhlglobaltracking.com` are received, parsed, threaded, and displayed live in the Admin Panel inbox.
...
[MX Record / Inbound Mail Server] (Cloudflare Email Routing or Resend Inbound)
         │
         ▼
[POST Webhook Endpoint] -> https://www.dhlglobaltracking.com/api/inbound-email
         │
...
3. Set **Endpoint URL**: `https://tracking-website-service.onrender.com/api/inbound-email` (or your custom domain URL `https://www.dhlglobaltracking.com/api/inbound-email`).
...
1. In [Cloudflare Dashboard](https://dash.cloudflare.com) for `dhlglobaltracking.com`, go to **Email Routing**.
2. Create a catch-all or specific rule for `support@dhlglobaltracking.com`.
3. Set action to **Send to Worker**.
4. Use a Cloudflare Worker script to POST the JSON payload to `https://www.dhlglobaltracking.com/api/inbound-email` with header:
   ```http
